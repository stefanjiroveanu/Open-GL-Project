...
uniform mat4 waveModel;
uniform mat4 waveView;
...
void main()
{
	...
	//compute normal
	fNormal = normalize(waveNormalMatrix * cross(bitangent, tangent));

	//compute position in eye space
	fPosition = vec3(waveView * waveModel * vec4(x, y, z, 1.0f));

	fragPosLightSpace = lightSpaceTrMatrix * waveModel * vec4(x, y, z, 1.0f);

	fragPosWorld = waveModel * vec4(x, y, z, 1.0f);

	gl_Position = waveProjection * waveView * waveModel * vec4(x, y, z, 1.0f);
}
